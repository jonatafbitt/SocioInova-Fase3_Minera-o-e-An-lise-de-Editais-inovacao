...
GERADOS em memória com pypdf (nada de binário commitado).
"""
...
# -- fixtures de PDF com docinfo geradas em memória --------------------------------
...
def _html_lista(links: list[tuple[str, str]]) -> str:
...
def _descobrir_coletar(cli, politeness_veloz, servidor_fake, *, html: str, pdfs: dict) -> None:
...
def _fila_rows(caminho_manifesto) -> list[dict]:
...
def _documents_unica(caminho_manifesto) -> dict:
    linhas = _documentos(caminho_manifesto)
    assert len(linhas) == 1
    return linhas[0]
...
def test_ler_metadados_tolerante(tmp_path):
    ...
    valido.write_bytes(
        pdf_com_docinfo([_longo("Doc com docinfo")], criado_em="D:20230101000000Z")
    )
    meta = ler_metadados(valido)
    assert meta is not None and meta["criado_em"].startswith("D:2023")

    sem_docinfo = tmp_path / "nu.pdf"
    sem_docinfo.write_bytes(pdf_com_docinfo([_longo("Sem docinfo nenhum")]))
...
def test_convergencia_aceita_com_tres_evidencias_e_metodo_url(
    ...
    _descobrir_coletar(
        ...
        politeness_veloz,
        servidor_fake,
        html=_html_lista([("/2023/edital-a.pdf", "Edital de Inovação 2023")]),
        pdfs={
            "/2023/edital-a.pdf": pdf_com_docinfo(
                [_longo("Edital convergente")],
    ...
    assert "aceitos: 1" in _saida(resultado)
    assert "fila: 0" in _saida(resultado)

    documento = _documentos(politeness_veloz.manifesto)[0]
...
def test_wiring_ancora_descoberta_persiste_integral_e_evento_trunca(
    ...
    _descobrir_coletar(
        ...
        politeness_veloz,
        servidor_fake,
        html=_html_lista([("/2023/com-link.pdf", ancora_longa)]),
        pdfs={
            "/2023/com-link.pdf": pdf_com_docinfo(
                [_longo("Wiring da ancora")], criado_em="D:20230101000000Z"
            )
        },
    )
    ...
    _coletar(
        ...
        servidor_fake,
        {
            "/2023/direto.pdf": pdf_com_docinfo(
                [_longo("Candidato registrado a mao")], criado_em="D:20230101000000Z"
            )
        },
    )
    with Manifesto(politeness_veloz.manifesto) as manifesto:
...
def test_so_url_sem_corroboracao_vai_a_fila_baixa_confianca(
    ...
    _coletar(
        ...
        politeness_veloz,
        servidor_fake,
        {"/2022/b.pdf": pdf_com_docinfo([_longo("Sem ano no docinfo")])},
    )

    resultado = cli.invoke(app, ["datar", "--portal", "TST"])

    assert resultado.exit_code == 0, _saida(resultado)
    assert "fila: 1" in _saida(resultado)

    documento = _documentos(politeness_veloz.manifesto)[0]
    assert documento["metodo_datacao"] is None and documento["ano_aceito"] is None

    itens = _fila_rows(politeness_veloz.manifesto)
    assert len(itens) == 1
    assert itens[0]["motivo"] == "baixa_confianca_sourl"
    assert itens[0]["status"] == "pendente"
    assert itens[0]["url_origem"] == documento["url_origem"]

    filas = [detalhe for tipo, detalhe in _tipos_eventos(politeness_veloz.manifesto) if tipo == "datacao_fila"]
    assert len(filas) == 1
...
def test_divergencia_vai_a_fila_com_sinal_de_qualidade(
    ...
    _descobrir_coletar(
        ...
        politeness_veloz,
        servidor_fake,
        html=_html_lista([("/2021/c.pdf", "Edital de Inovação 2023")]),
        pdfs={
            "/2021/c.pdf": pdf_com_docinfo(
                [_longo("Divergencia entre fontes")], criado_em="D:20210215000000Z"
            )
        },
    )
...
def test_ano_limite_so_url_2026_vai_a_fila(cli, politeness_veloz, servidor_fake):
    _coletar(
        ...
        politeness_veloz,
        servidor_fake,
        {"/2026/d.pdf": pdf_com_docinfo([_longo("Limite da janela sem corroboracao")])},
    )
    ...
    _coletar(
        ...
        servidor_fake,
        {
            "/limite/f.pdf": pdf_com_docinfo(
                [_longo("Limite com corroboracao interna")],
    ...
    _coletar(
        ...
        servidor_fake,
        {
            "/docs/e.pdf": pdf_com_docinfo(
                [_longo("Publicacao antiga fora da janela")],
...
def test_sem_data_alguma_enfileira_sem_descartar(cli, politeness_veloz, servidor_fake):
    _coletar(
        ...
        politeness_veloz,
        servidor_fake,
        {"/docs/g.pdf": pdf_com_docinfo([_longo("Nenhuma fonte produz ano")])},
    )
...
def test_reexecucao_idempotente_pula_datados_e_preserva_fila(
    ...
    _descobrir_coletar(
        ...
        pdfs={
            "/2023/a.pdf": pdf_com_docinfo(
                [_longo("Convergente para aceite")], criado_em="D:20230101000000Z"
            ),
            "/docs/b.pdf": pdf_com_docinfo([_longo("Vai para a fila sem data")]),
        },
    )

    primeira = cli.invoke(app, ["datar", "--portal", "TST"])
    assert primeira.exit_code == 0, primeira.output
    assert "aceitos: 1" in _saida(primeira) and "fila: 1" in _saida(primeira)
    eventos_primeira = [tipo for tipo, _ in _tipos_eventos(politeness_veloz.manifesto)]
    ...
    saida = _saida(segunda)
    assert "pulados: 2" in saida, "resumo mostra os pulados"
    assert "aceitos: 0" in saida and "fila: 0" in saida
    eventos_segunda = [tipo for tipo, _ in _tipos_eventos(politeness_veloz.manifesto)]
    ...
    assert len(_fila_rows(politeness_veloz.manifesto)) == 1, "fila permanece intacta"
...
def test_decisao_humana_impede_reprocessamento_na_retomada(
    ...
    _coletar(
        ...
        politeness_veloz,
        servidor_fake,
        {"/docs/r.pdf": pdf_com_docinfo([_longo("Fila decidida por humana")])},
    )
    assert cli.invoke(app, ["datar", "--portal", "TST"]).exit_code == 0
    (item,) = _fila_rows(politeness_veloz.manifesto)
    ...
    assert "pulados: 1" in _saida(retomada), "decisão humana não é refeta"
    assert len(_fila_rows(politeness_veloz.manifesto)) == 1, "nenhum item duplicado"
...
def test_pdf_corrompido_ou_sumido_vira_datacao_erro_e_lote_segue(
    ...
    _coletar(
        ...
        servidor_fake,
        {
            "/quebrado/x.pdf": pdf_com_docinfo(
                [_longo("Este sera corrompido depois")], criado_em="D:20220101000000Z"
            ),
            "/sumido/s.pdf": pdf_com_docinfo(
                [_longo("Este sera apagado depois")], criado_em="D:20220101000000Z"
            ),
            "/integro/i.pdf": pdf_com_docinfo(
                [_longo("Integro e convergente")], criado_em="D:20230101000000Z"
            ),
        },
    )
    ...
    assert resultado.exit_code == 0, _saida(resultado) + " — lote NUNCA aborta"
...
def test_alias_e_datado_como_linha_propria(cli, politeness_veloz, servidor_fake):
    corpo = pdf_com_docinfo(
        [_longo("Mesmos bytes em duas urls com docinfo")], criado_em="D:20230101000000Z"
    )
    _coletar(
        cli,
        politeness_veloz,
    ...
    assert resultado.exit_code == 0, resultado.output
    assert "aceitos: 2" in _saida(resultado)
...
def _enfileirar_um(cli, politeness_veloz, servidor_fake) -> dict:
    _coletar(
        ...
        politeness_veloz,
        servidor_fake,
        {"/docs/q.pdf": pdf_com_docinfo([_longo("Sem data para a fila")])},
    )
    assert cli.invoke(app, ["datar", "--portal", "TST"]).exit_code == 0
    return _fila_rows(politeness_veloz.manifesto)[0]
...
def test_fila_decidir_ano_grava_decisao_completa_e_evento(
    ...
    assert resultado.exit_code == 0, resultado.output
    (decidido,) = _fila_rows(politeness_veloz.manifesto)
    ...
    eventos = [detalhe for tipo, detalhe in _tipos_eventos(politeness_veloz.manifesto) if tipo == "fila_decidida"]
...
def test_item_resolvido_recusa_segunda_decisao_exit_1(
    ...
    (unico,) = _fila_rows(politeness_veloz.manifesto)
...
def test_decisao_invalida_recusada_exit_2_nada_gravado(
    ...
    (item_depois,) = _fila_rows(politeness_veloz.manifesto)
...
def test_datar_documento_metodo_ancora_quando_url_nao_produz_ano(tmp_path):
    ...
    corpo = pdf_com_docinfo(
        [_longo("Ancora decide")], criado_em="D:20230101000000Z"
    )
...
def test_datar_documento_arquivo_ausente_vira_erro_leitura(tmp_path):
    corpo = pdf_com_docinfo([_longo("Sumira do disco")], criado_em="D:20230101000000Z")
    manifesto, documento = _manifesto_unitario(
...
def test_datar_documento_falha_de_persistencia_vira_erro_e_lote_segue(
    ...
    corpo = pdf_com_docinfo(
        [_longo("Persistencia recusada")], criado_em="D:20230101000000Z"
    )
    manifesto, documento = _manifesto_unitario(
        tmp_path, url="http://x.org/2023/doc.pdf", corpo=corpo
    )
...
def test_datar_documento_enfileiramento_recusado_vira_erro_fila_duplicada(
    ...
    corpo = pdf_com_docinfo([_longo("Sem data e sem vaga na fila")])
    manifesto, documento = _manifesto_unitario(
    ...
    _coletar(
        ...
        servidor_fake,
        {
            "/2023/off.pdf": pdf_com_docinfo(
                [_longo("Offline converge")], criado_em="D:20230101000000Z"
            )
        },
    )
