...
**Dicionário de stopwords (v4).** O filtro combina (a) base gramatical do português, (b) camada expandida de boilerplate de editais (seleção, homologação, coordenadoria, cronograma…) e (c) camada domínio-específica SocioInova — a enumeração completa dos **38 Institutos Federais** (`ifba`, `ifbaiano`, `ifrs`, `ifmg`, `ifnmg`, `ifsudestemg`, `ifsuldeminas`, `ifsertao`…) e demais siglas da Rede Federal (`cefet`, `cefetmg`, `cefetrj`, `utfpr`, `cp2`, `mec`, `setec`, `conif`), genéricos institucionais (`campus`, `reitoria`, `coordenadoria`…), burocracia normativa (`caput`, `diário oficial`, `considerando`, `disposto`…), temporalidade e formulário (meses, `cpf`, `cnpj`, `prazo`…), artefatos digitais (URLs, `gov`, `edu`…) e formas verbais frequentes (poder, dever, realizar…), mantidas também na variante sem acento para o texto resgatado por OCR. Termos analíticos de interesse (inovação, comunidade, território, licenciamento…) são preservados — a ficha técnica da sidebar registra a versão e o total de termos (rastreabilidade).
